function constraints = computeConstraints(sys, params, data, M_matrices, controller, stoch_constraints)
Mz0 = M_matrices.Mz0;
L = params.L;
n = sys.n;
m = sys.m;
...
for i=2:L+1
    Eps_pred = Hx((i-1)*n+1:i*n,:)*Alpha_eps;
...
%Reachable set:
CL = Ainv*(intersect(Xf,X{end}) + (-B)*U{L});
CL = intersect(CL,X{L});
...
Dist = Hx(n+1:2*n,:)*Alpha_d + Hx(n+1:2*n,:)*Alpha_eps + Eps; %Extended disturbance Polyhedron
...
for i = 1:imax
    ...
    CLnew = Ainv*(CLR - Dist + (-B)*U{1}); %One step backwards in the dynamics
...
if params.use_first_step_constraint
    ...
    constraints.polyinit = inv(sys.A)*(constraints.polyfs+((-sys.B)*sys.input.U));
